<script lang="ts">
	...
	let {
		size = 24,
		color = ['#18253F', '#274D82', '#3068B7'],
		class: className = undefined
	}: Props = $props();

	let sizePx = $derived(`${size}px`);
	let [color1, color2, color3] = $derived(Array.isArray(color) ? color : [color, color, color]);
</script>

<svg width={sizePx} height={sizePx} viewBox="0 0 256 203" class={className}>
	<defs>
		<path
	...
	<g transform="translate(2.000000, 1.510645)" fill-rule="nonzero">
		...
		<polygon
			fill={color2}
			points="149.426831 150.874561 96.0134271 150.874561 71.8889656 121.341138 140.252621 121.33896 140.255033 117.149462 179.332589 117.149462"
		/>
		<polygon
			fill={color2}
			points="223.55992 42.3226943 76.1782017 127.413686 56.9521852 103.361957 171.050895 37.4849931 168.955265 33.853745 199.34598 16.3076536"
		/>
		<polygon
			fill={color2}
		...
		/>
		<g>
			<use fill="black" fill-opacity="1" filter="url(#zod-filter-2)" xlink:href="#zod-path-1" />
			<use fill={color3} xlink:href="#zod-path-1" />
		</g>
	</g>
</svg>
